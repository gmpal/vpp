...
const BatteryManagement: React.FC = () => {
  ...
  async function handleAddBatteryFromDialog() {
    try {
      const newBattery = await addBattery(
        capacity,
        currentSoc,
        maxCharge,
        maxDischarge,
  ...
  async function handleRemoveBattery(batteryId: string) {
    try {
      ...
      setSnackbar({
        open: true,
        message: "Failed to remove battery.",
        severity: "error",
      });
    }
  }
  ...
  async function handleCharge(
    ...
    try {
      const updated = await chargeBattery(batteryId, {
        power_kW: power,
        duration_h: duration,
      });
      setBatteries((prev) =>
        prev.map((b) => (b.battery_id === batteryId ? updated : b)),
      );
      setSnackbar({
        open: true,
        message: `Battery ${batteryId} charged successfully!`,
        severity: "success",
      });
    } catch (error) {
      ...
      setSnackbar({
        open: true,
        message: `Failed to charge Battery ${batteryId}.`,
        severity: "error",
      });
    }
  }
  ...
  async function handleDischarge(
    ...
    try {
      const updated = await dischargeBattery(batteryId, {
        power_kW: power,
        duration_h: duration,
      });
      setBatteries((prev) =>
      ...
      setSnackbar({
        open: true,
        message: `Battery ${batteryId} discharged successfully!`,
        severity: "success",
      });
    } catch (error) {
      ...
      setSnackbar({
        open: true,
        message: `Failed to discharge Battery ${batteryId}.`,
        severity: "error",
      });
    }
  }
    ...
    <Box my={4}>
      ...
      <Dialog open={openBatteryDialog} onClose={handleCloseBatteryDialog}>
        ...
        <DialogActions>
          ...
          <Button
            onClick={handleAddBatteryFromDialog}
            variant="contained"
            color="primary"
          >
            Add Battery
          ...
          {batteries.map((battery) => (
            <Grid item xs={12} md={6} key={battery.battery_id}>
              <Paper elevation={3} sx={{ p: 2 }}>
                ...
                <Typography>
                  Max Charge Rate: {battery.max_charge_kW} kW
                </Typography>
                <Typography>
                  Max Discharge Rate: {battery.maxDischarge_kW} kW
                </Typography>
                ...
                <Box mt={2} display="flex" gap={2}>
                  <Button
                    ...
                    color="success"
                    onClick={() =>
                      handleOpenActionDialog("charge", battery.battery_id)
                    }
                  >
                    Charge
                  </Button>
                  <Button
                    ...
                    color="error"
                    onClick={() =>
                      handleOpenActionDialog("discharge", battery.battery_id)
                    }
                  >
                    Discharge
                  </Button>
