...
describe('EVCard', () => {
  ...
  it('shows SOC percentage in text', () => {
    ...
    render(<EVCard ev={makeEV({ soc_kwh: 37.5, capacity_kwh: 75 })} onRefresh={jest.fn()} />);
    expect(screen.getByText(/50%/)).toBeInTheDocument();
  });

  it('shows correct SOC text for high SOC (>70%)', () => {
    // 60/75 = 80%
    render(<EVCard ev={makeEV({ soc_kwh: 60, capacity_kwh: 75 })} onRefresh={jest.fn()} />);
    expect(screen.getByText(/80%/)).toBeInTheDocument();
  });

  it('shows correct SOC text for low SOC (<30%)', () => {
    // 15/75 = 20%
    render(<EVCard ev={makeEV({ soc_kwh: 15, capacity_kwh: 75 })} onRefresh={jest.fn()} />);
  ...
  it('calls chargeVehicle with correct vehicle_id and calls onRefresh', async () => {
    mockApi.chargeVehicle.mockResolvedValue({ vehicle_id: 'ev_test1', new_soc_kwh: 50 });
    ...
    await waitFor(() =>
      expect(mockApi.chargeVehicle).toHaveBeenCalledWith('ev_test1', expect.any(Number))
    );
    await waitFor(() => expect(onRefresh).toHaveBeenCalled());
  });

  it('calls dischargeVehicle and onRefresh on discharge button click', async () => {
    mockApi.dischargeVehicle.mockResolvedValue({ vehicle_id: 'ev_test1', new_soc_kwh: 30 });
    const onRefresh = jest.fn();
    ...
    await waitFor(() =>
      expect(mockApi.dischargeVehicle).toHaveBeenCalledWith('ev_test1', expect.any(Number))
    );
    await waitFor(() => expect(onRefresh).toHaveBeenCalled());
  });
  ...
  it('charge button is disabled when SOC is at 100%', () => {
    // 75/75 = 100%
    render(<EVCard ev={makeEV({ soc_kwh: 75, capacity_kwh: 75 })} onRefresh={jest.fn()} />);
    fireEvent.click(screen.getAllByRole('button')[0]);
