...
describe('HouseholdPanel', () => {
  ...
  it('shows empty state when no households', async () => {
    render(<HouseholdPanel />);
    await waitFor(() =>
      expect(screen.getByText('No households yet. Add one to get started.')).toBeInTheDocument()
    );
  });
  ...
  it('Add EV button is disabled when no households', async () => {
    render(<HouseholdPanel />);
    await waitFor(() => screen.getByText('No households yet. Add one to get started.'));
  ...
  it('opens Add Household dialog when button is clicked', async () => {
    ...
    expect(screen.getByText('Add Household', { selector: 'h2' })).toBeInTheDocument();
  });
  ...
  it('calls createHousehold and refreshes on dialog submit', async () => {
    mockApi.createHousehold.mockResolvedValue({
      household_id: 'hh_new1', name: 'New House', latitude: 50.85, longitude: 4.35,
    });
    mockApi.getHouseholds.mockResolvedValueOnce([])
      .mockResolvedValueOnce([{ household_id: 'hh_new1', name: 'New House', latitude: 50.85, longitude: 4.35 }]);
  ...
  it('calls deleteHousehold when delete button is clicked', async () => {
    ...
    await waitFor(() => expect(mockApi.deleteHousehold).toHaveBeenCalledWith('hh_abc1'));
  });
});
