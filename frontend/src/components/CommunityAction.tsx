...
const ACTION_CONFIG: Record<
  string,
  { icon: React.ReactNode; color: string; getMessage: (s: any) => string }
> = {
  selling: {
  ...
  buying: {
    ...
    color: "#ff9800",
    getMessage: (s) =>
      `Buying ${Math.abs(s.net).toFixed(2)} kW — consider charging your EVs`,
  },
  charging_evs: {
...
const CommunityAction: React.FC = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        setSummary(await getCommunitySummary());
      } catch {}
    };
    fetch();
