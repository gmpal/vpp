...
describe('CommunityDashboard', () => {
  ...
  it('renders Community Dashboard heading', () => {
  ...
  it('shows stat card titles before data loads', () => {
    mockApi.getCommunitySummary.mockImplementation(() => new Promise(() => {}));
    render(<CommunityDashboard />);
  ...
  it('hides EV SOC bar when ev_count === 0', async () => {
    mockApi.getCommunitySummary.mockResolvedValue({ ...mockSummary, ev_count: 0 });
    render(<CommunityDashboard />);
    await waitFor(() => screen.getByText('10.00 kW'));
    expect(screen.queryByText('Fleet EV State of Charge')).not.toBeInTheDocument();
  });

  it('renders live power chart', () => {
    mockApi.getCommunitySummary.mockResolvedValue(mockSummary);
    render(<CommunityDashboard />);
    expect(screen.getByText('Live Power (last 30 readings)')).toBeInTheDocument();
  });
  ...
  it('shows error alert when fetch fails', async () => {
    ...
    render(<CommunityDashboard />);
    await waitFor(() =>
      expect(screen.getByText('Could not load community data')).toBeInTheDocument()
    );
  });
