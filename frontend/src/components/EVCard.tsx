...
const EVCard: React.FC<EVCardProps> = ({ ev, onRefresh }) => {
  ...
  const socColor = socPct > 70 ? '#4caf50' : socPct > 30 ? '#ff9800' : '#f44336';

  return (
    <Paper variant="outlined" sx={{ mt: 1.5, p: 1.5, borderRadius: 2 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
        <ElectricCarIcon sx={{ color: ev.status === 'home' ? '#2196f3' : '#ff9800', fontSize: 20 }} />
        <Typography variant="body2" fontWeight="bold" sx={{ flex: 1 }}>{ev.name}</Typography>
        <Chip
          label={ev.status}
          size="small"
        ...
        <IconButton size="small" color="error" onClick={async () => {
          await deleteVehicle(ev.vehicle_id); onRefresh();
        }}>
      ...
      <Box sx={{ mt: 1 }}>
        ...
        <LinearProgress
          variant="determinate"
          value={socPct}
          sx={{ height: 8, borderRadius: 4, bgcolor: '#e0e0e0', '& .MuiLinearProgress-bar': { bgcolor: socColor } }}
        />
      </Box>

      <Collapse in={expanded}>
        <Box sx={{ mt: 1.5 }}>
          <Typography variant="caption" color="text.secondary">Power (kW)</Typography>
          <Slider
            value={power}
            min={0}
          ...
          <Box sx={{ display: 'flex', gap: 1, mt: 1 }}>
            <Button
              size="small" variant="contained" startIcon={<BoltIcon />}
              onClick={async () => { await chargeVehicle(ev.vehicle_id, power); onRefresh(); }}
              disabled={socPct >= 100}
              color="primary"
              sx={{ flex: 1 }}
            >
            ...
            <Button
              ...
              onClick={async () => { await dischargeVehicle(ev.vehicle_id, power); onRefresh(); }}
              disabled={socPct <= 0}
              color="warning"
              sx={{ flex: 1 }}
            >
