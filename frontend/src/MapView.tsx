...
import {
  Box, Typography, IconButton, Chip, Button, Dialog, DialogTitle, DialogContent,
  DialogActions, TextField, Select, MenuItem, FormControl, InputLabel,
  Drawer, List, ListItem, ListItemButton, ListItemIcon, ListItemText, Divider, Paper,
...
const AddSourceDialog: React.FC<AddSourceDialogProps> = ({ open, position, onClose, onAdd }) => {
    ...
    <Dialog open={open} onClose={onClose}>
      ...
      <DialogContent>
        <Typography variant="body2" color="textSecondary" gutterBottom>
          Location: {position ? `${position[0].toFixed(4)}, ${position[1].toFixed(4)}` : 'N/A'}
        </Typography>
        ...
        <TextField
          ...
          placeholder={`${sourceType.charAt(0).toUpperCase() + sourceType.slice(1)} Source`}
        />
      </DialogContent>
      <DialogActions>
...
const MapView: React.FC = () => {
  ...
  const handleGenerateSystemData = async () => {
    try {
      ...
      alert('System data generation started! Load and market data will be available shortly.');
    } catch (error) {
  ...
  const handleDeleteSource = async (sourceId: string) => {
    if (!window.confirm('Are you sure you want to delete this energy source?')) return;
  ...
  const totalProduction = sources.reduce((sum, s) => sum + (s.current_value || 0), 0);
                ...
                <Box display="flex" alignItems="center" justifyContent="space-between" width="100%">
                  <Box display="flex" alignItems="center" gap={1}>
                    ...
                    <Typography variant="body2" fontWeight="bold">
                      {source.name || `${source.source_type} ${source.source_id.substring(0, 6)}`}
                    </Typography>
                  </Box>
                  <IconButton size="small" onClick={() => handleDeleteSource(source.source_id)} color="error">
                  ...
                  <Box sx={{ minWidth: 200 }}>
                    <Box display="flex" alignItems="center" justifyContent="space-between" mb={1}>
                      <Box display="flex" alignItems="center" gap={1}>
                        ...
                        <Typography variant="h6">
                          {source.name || `${source.source_type} ${source.source_id}`}
                        </Typography>
                      </Box>
                    </Box>
                    ...
                    {source.current_value !== null && (
                      <Typography variant="body2" color="primary" sx={{ mt: 1 }}>
                        <strong>Current Production:</strong> {source.current_value.toFixed(2)} kW
                      </Typography>
                    )}
                  </Box>
                </Popup>
          ...
          <Paper sx={{ p: 2, mt: 2, backgroundColor: '#e3f2fd' }}>
            ...
            <Box display="flex" flexDirection="column" gap={1.5} mt={1}>
              ...
              <Button
                ...
                size="small"
              >
                {trainingStatus.is_running_inference ? 'Running...' : 'Run Inference'}
              </Button>
